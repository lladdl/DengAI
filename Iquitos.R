...
train_iq$week <- as.numeric(format(train_iq$week_start_date, "%U"))  # week number (0–52)
...
# Create time slices
time_slices <- create_time_slices(train_iq, initial_size = 16, horizon = 1)

# Prepare PCA predictor columns
pca_vars <- c(grep("^(veg|precip|temp)_PC", names(train_iq), value = TRUE),"sin_week", "cos_week")
